...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#bulk_delete_btn').on('click', function() {
            if (selectedIds.length === 0) {
                toastr.warning('Please select at least one inquiry to delete');
                return;
            }

            Swal.fire({
                title: 'Are you sure?',
                text: 'You will delete ' + selectedIds.length + ' inquiry(ies)!',
                icon: 'warning',
        ...
        function bulkDeleteInquiries(ids) {
            $.ajax({
                type: "POST",
                url: "{{route('bulk.delete.inquiry')}}",
                data: {
                    ids: ids,
                    _token: '{{csrf_token()}}'
                },
